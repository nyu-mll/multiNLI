...
class MyModel(object):
    def __init__(self, seq_length, emb_dim, hidden_dim, embeddings):
        ...
        self.W_v1 = tf.Variable(tf.random_normal([self.dim * 4, self.dim], stddev=0.1))
        self.b_v1 = tf.Variable(tf.random_normal([self.dim], stddev=0.1))

        self.W_v2 = tf.Variable(tf.random_normal([self.dim * 4, self.dim], stddev=0.1))
        self.b_v2 = tf.Variable(tf.random_normal([self.dim], stddev=0.1))
        
        self.W_mlp = tf.Variable(tf.random_normal([self.dim * 2, self.dim], stddev=0.1))
        ...
        ### Subcomponent Inference: No differnece of element-wise product ###

        m_a = []
        m_b = []
        
        for i in range(self.sequence_length):
            m_a_i = tf.concat([premise_list[i], premise_attn[i]], 1)
            m_b_i = tf.concat([hypothesis_list[i], hypothesis_attn[i]], 1)
            m_a.append(m_a_i)
            m_b.append(m_b_i)
        ...
        for i in range(self.sequence_length):
            v1_i = tf.matmul(m_a[i], self.W_v1) + self.b_v1
            v1.append(v1_i)
            v2_i = tf.matmul(m_b[i], self.W_v2) + self.b_v2
            v2.append(v2_i)

        V_1 = tf.stack(v1, axis=1)
        V_2 = tf.stack(v2, axis=1)


        ### Parikh-like Summation layer ###

        v_1_sum = tf.reduce_sum(V_1, 1)
        v_2_sum = tf.reduce_sum(V_2, 1)

        v = tf.concat([v_1_sum, v_2_sum], 1)
